...
IMAGE="sudoku-app:${REV}-${NODE_ARCH}"
TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT INT TERM
sed "s|image: sudoku-app:0.2.0|image: $IMAGE|" sudoku-deployment.yaml > "$TMP/resources.yaml"
...
STATE_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/nicqx-apps/$TARGET/sudoku-app"
mkdir -p -m 700 "$STATE_DIR"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
$KUBECTL get deployment/sudoku-app service/sudoku-app -n "$NAMESPACE" -o yaml \
...
$KUBECTL rollout status deployment/sudoku-app -n "$NAMESPACE" --timeout=180s
echo "Ready: Deployment/sudoku-app ($IMAGE)"
